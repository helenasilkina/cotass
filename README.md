...
<a href="https://www.youtube.com/watch?v=7ebV-1RLq-A&index=59&list=PLuWypj7F_mQlIBhQwfb8qgRjI-qZuJx1U">Кратчайшие пути против матричного умножения</a>

Предположим, что у вас есть ориентированный граф на n вершинах и вам требуется найти кратчайшие попарные расстояния в нем. Для простоты будем считать, что длины ребер единичные. Можно запустить n поисков в ширины или один алгоритм Флойда и найти ответы за O(n^3) времени. Но можно ли быстрее? Оказывается, задачу про кратчайшие пути все же можно решить, если запастись обычным (целочисленным) матричным умножением, монеткой и терпением.
<a href="https://cotass.wordpress.com/2016/10/20/%d0%ba%d1%80%d0%b0%d1%82%d1%87%d0%b0%d0%b9%d1%88%d0%b8%d0%b5-%d0%bf%d1%83%d1%82%d0%b8-%d0%bf%d1%80%d0%be%d1%82%d0%b8%d0%b2-%d0%bc%d0%b0%d1%82%d1%80%d0%b8%d1%87%d0%bd%d0%be%d0%b3%d0%be-%d1%83%d0%bc/">Источник</a>

<a href="https://www.youtube.com/watch?v=H1bEB78SNOs&list=PLuWypj7F_mQlIBhQwfb8qgRjI-qZuJx1U&index=60">О паросочетаниях в регулярных двудольных графах</a>

Эффективные алгоритмы для поиска максимальных паросочетаний в двудольных графах известны еще в первой половины 20 века. Большинство их них, однако, касаются случая графа произвольного вида. Еще во времена Холла, однако, было известно, что если граф регулярный, то в нем непременно существует совершенное паросочетание. Этот факт, впрочем, долгое время не давал никаких алгоритмических плодов. В начале 2000-х годов ситуация изменилась: появилась серия работ, в которой были представлены эффективные, элегантные и простые алгоритмы, решающие данную задачу за существенно меньшее время. В частности оказалось, что совершенное паросочетание в двудольном графе можно найти за линейное время O(E), а если разрешить рандомизацию, то даже за O(V log V).
<a href="https://cotass.wordpress.com/2016/11/17/%d0%be-%d0%bf%d0%b0%d1%80%d0%be%d1%81%d0%be%d1%87%d0%b5%d1%82%d0%b0%d0%bd%d0%b8%d1%8f%d1%85-%d0%b2-%d1%80%d0%b5%d0%b3%d1%83%d0%bb%d1%8f%d1%80%d0%bd%d1%8b%d1%85-%d0%b4%d0%b2%d1%83%d0%b4%d0%be%d0%bb/">Источник</a>

<a href="https://www.youtube.com/watch?v=pT1r-VqncDk&index=61&list=PLuWypj7F_mQlIBhQwfb8qgRjI-qZuJx1U">Методы безопасного освобождения памяти в lock-free структурах данных (Андрей Кашин 2016.11.23)</a>

При попытке реализовать lock-free структуру данных любой программист рано или поздно сталкивается с вопросом «Когда можно освободить память, занимаемую удаленными объектами?». Оказывается, при наивных попытках ответить на этот вопрос возникает много проблем (ABA, NullPointerException), и для подобных структур данных простого ответа на этот вопрос просто нет. Зато существуют более сложные и принципиально различные методы для безопасного освобождения памяти, каждый из которых обладает своими преимуществами и недостатаками, которые надо принимать во внимание при выборе того или иного метода. На семинаре Андрей Кашин познакомит с двумя такими методами: Hazard pointers (опасные указатели), RCU (Read-copy-update). А далее будут рассказаны теоретические свойства этих методов, а затем посмотрим на интересные детали реализации в реальных библиотеках.
<a href="https://cotass.wordpress.com/2016/11/25/%d0%bc%d0%b5%d1%82%d0%be%d0%b4%d1%8b-%d0%b1%d0%b5%d0%b7%d0%be%d0%bf%d0%b0%d1%81%d0%bd%d0%be%d0%b3%d0%be-%d0%be%d1%81%d0%b2%d0%be%d0%b1%d0%be%d0%b6%d0%b4%d0%b5%d0%bd%d0%b8%d1%8f-%d0%bf%d0%b0%d0%bc/">Источник</a>

<a href="https://www.youtube.com/watch?v=vI-Lujlxe20&list=PLuWypj7F_mQlIBhQwfb8qgRjI-qZuJx1U&index=62">Подсчет суммы элементов в потоке в окне фиксированного размера (Колесниченко Игнат 2016.11.30)</a>

Рассмотрим такую задачу, есть поток числе a1, a2 ..., которые поступают к нам по-одному и дано число n. Требуется придумать структуру данных, для поддержания суммы последних n чисел в потоке. На первый взгляд задача кажется неразрешимой, если у нас имеется лишь o(n) памяти, даже если требуется дать ответ приближенно.
<a href="https://cotass.wordpress.com/2016/12/01/%d0%bf%d0%be%d0%b4%d1%81%d1%87%d0%b5%d1%82-%d1%81%d1%83%d0%bc%d0%bc%d1%8b-%d1%8d%d0%bb%d0%b5%d0%bc%d0%b5%d0%bd%d1%82%d0%be%d0%b2-%d0%b2-%d0%bf%d0%be%d1%82%d0%be%d0%ba%d0%b5-%d0%b2-%d0%be%d0%ba%d0%bd/">Источник</a>

<a href="https://www.youtube.com/watch?v=OkWmfr62VJ8&list=PLuWypj7F_mQlIBhQwfb8qgRjI-qZuJx1U&index=63">Maximally recoverable codes for distributed data storage (Сергей Еханин 2016.12.07)</a>

Data storage applications require erasure-correcting codes with prescribed sets of dependencies between data symbols and parity symbols (topology). A code as above is Maximally Recoverable (MR), if it corrects all erasure patterns that are information theoretically correctable given the dependency constraints. Applications furthermore need codes over small finite fields in order to facilitate encoding and decoding operations. In this talk we briefly survey the state of the art erasure codes for distributed data storage and then focus on the recent progress in constructions and lower bounds for MR codes. This is a joint work with Parikshit Gopalan, Guangda Hu, Swastik Kopparty, Shubhangi Saraf, and Carol Wang.
<a href="https://cotass.wordpress.com/2016/12/08/maximally-recoverable-codes-for-distributed-data-storage-%d1%81%d0%b5%d1%80%d0%b3%d0%b5%d0%b9-%d0%b5%d1%85%d0%b0%d0%bd%d0%b8%d0%bd-2016-12-07/">Источник</a>

<a href="https://www.youtube.com/watch?v=UfcGWkgTtPE&list=PLuWypj7F_mQlIBhQwfb8qgRjI-qZuJx1U&index=64">Fast Parallel Operations on Search Trees (Ярослав Ахремцев 2016.12.28)</a>
...
<a href="https://cotass.wordpress.com/2017/01/01/fast-parallel-operations-on-search-trees-%d1%8f%d1%80%d0%be%d1%81%d0%bb%d0%b0%d0%b2-%d0%b0%d1%85%d1%80%d0%b5%d0%bc%d1%86%d0%b5%d0%b2-2016-12-28/">Источник</a>

<a href="https://www.youtube.com/watch?v=V3iRRZZTPDM&list=PLuWypj7F_mQlIBhQwfb8qgRjI-qZuJx1U&index=65">Однопроходный онлайн алгоритм поиска плотной компоненты</a>

В данном докладе будет разбираться задача о поиске плотной компоненты в неориентированном графе, точнее её аппроксимация. Плотность подграфа определяется как отношение числа рёбер к числу вершин. Поиск аппроксимации плотной компоненты осуществляется с помощью декомпозиции множества вершин на вложенные в друг друга подмножества. Для построения данной декомпозиции, в статье, предложены несколько алгоритмов: потоковый вероятностный алгоритм (алгоритм обрабатывает только случайное подмножество рёбер размера O(V logV)), динамический алгоритм (алгоритм, способный обрабатывать добавление и удаление ребра за O(logV)) и одновременно потоковый и динамический алгоритм (данный алгоритм, скорее всего рассмотрен не будет, но он заимствует много идей из предыдущих двух). Доклад делается по статье Space- and Time-Efficient Algorithm for Maintaining Dense Subgraphs on One-Pass Dynamic Streams.
<a href="https://cotass.wordpress.com/2017/02/21/%d0%be%d0%b4%d0%bd%d0%be%d0%bf%d1%80%d0%be%d1%85%d0%be%d0%b4%d0%bd%d1%8b%d0%b9-%d0%be%d0%bd%d0%bb%d0%b0%d0%b9%d0%bd-%d0%b0%d0%bb%d0%b3%d0%be%d1%80%d0%b8%d1%82%d0%bc-%d0%bf%d0%be%d0%b8%d1%81%d0%ba/">Источник</a>

<a href="https://www.youtube.com/watch?v=j90jllSjgAU&index=66&list=PLuWypj7F_mQlIBhQwfb8qgRjI-qZuJx1U">Несколько сюжетов об инверсиях</a>

Инверсии – классическое понятие комбинаторики, упоминание о которых можно найти в любом учебнике. С точки зрения computer science знакомство с инверсиями обычно заканчивается на алгоритме, вычисляющем их число в перестановке за время O(n \log n). В данном докладе Максим Бабенко попробует пойти дальше и рассказать несколько интересных, но менее известных фактов про них. В начале будет доказано, что количество инверсий в перестановке определяет сложность ее сортировки: существует алгоритм, сортирующий перестановку за время O(n \log{k/n}), где k — количество инверсий в ней. Далее, Максим Бабенко обратится к самому вопросу вычисления k. Оказывается, что в RAM-модели вычислений количество инверсий может быть вычислено быстрее. Этот прорыв был получен Chan и Pătraşcu в работе «Counting Inversions, Offline Orthogonal Range Counting, and Related Problems».
<a href="https://cotass.wordpress.com/2017/03/04/%d0%bd%d0%b5%d1%81%d0%ba%d0%be%d0%bb%d1%8c%d0%ba%d0%be-%d1%81%d1%8e%d0%b6%d0%b5%d1%82%d0%be%d0%b2-%d0%be%d0%b1-%d0%b8%d0%bd%d0%b2%d0%b5%d1%80%d1%81%d0%b8%d1%8f%d1%85-%d0%b4%d0%be%d0%ba%d0%bb%d0%b0/">Источник</a>

<a href="https://www.youtube.com/watch?v=z1g4be9ixMY&index=67&list=PLuWypj7F_mQlIBhQwfb8qgRjI-qZuJx1U">О поддержании порядка в списке</a>

Максим Бабенко рассмотрит простую структуру данных, позволяющую поддерживать связный список с операциями вставки после заданного элемента, удаления заданного элемента, а также дополнительной функцией Order(x,y), которая выясняет, какой из элементов x, y идет в списке первым.
<a href="https://cotass.wordpress.com/2017/03/12/%d0%be-%d0%bf%d0%be%d0%b4%d0%b4%d0%b5%d1%80%d0%b6%d0%b0%d0%bd%d0%b8%d0%b8-%d0%bf%d0%be%d1%80%d1%8f%d0%b4%d0%ba%d0%b0-%d0%b2-%d1%81%d0%bf%d0%b8%d1%81%d0%ba%d0%b5-%d0%b4%d0%be%d0%ba%d0%bb%d0%b0%d0%b4/">Источник</a>
